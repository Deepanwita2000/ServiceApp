...
class AllServices(ModelViewSet):
    queryset :QuerySet= Service.objects.all()
    ...
    search_fields = [
        'name',                # service name
                               # service description
        'providers__location', # search by provider's location
        'providers__pincode',  # search by provider's pincode
    ]
...
# View all services without authentication
    @action(detail=False, methods=['get'], url_path='all-services', permission_classes=[AllowAny] , authentication_classes=[])
