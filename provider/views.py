...
class ProviderRegistrationSet(ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='my-profile', permission_classes=[IsProvider])
    def my_profile(self, request):
        user = request.user
        if user.role != 'provider':
            raise PermissionDenied("Only provider can access their own profile.")

        provider = Provider.objects.filter(provider=user)
        serializer = self.get_serializer(provider, many=True)
...
class ViewBookings(ModelViewSet):
    ...
    def my_bookings(self,request):
        ...
        #     raise PermissionDenied("You are not yet approved!")
        try:
            provider = Provider.objects.get(provider=user)
            print("provider -> ",provider)
            if provider.is_approved != True:
                raise PermissionDenied("You are not yet approved!")
            print(provider.is_approved)
